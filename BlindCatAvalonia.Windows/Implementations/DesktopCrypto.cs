...
public class DesktopCrypto : Crypto
{
    ...
    protected override async Task<AppResponse> EncodeVideoTo_Mp4_CENC(string inputFile, string target, string password)
    {
    ...
    protected override async Task<AppResponse> EncodeVideoTo_Mp4_CENC(Stream inputStream, string target, string password)
    {
        try
        {
