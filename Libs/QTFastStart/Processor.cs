...
namespace QTFastStart
{
    public class Processor
    {
        ...
        private Atom ReadAtomEx(BinaryReader dataStream)
        {
            ...
            long ln = position + size;
            if (ln > dataStream.BaseStream.Length)
            {
                size = dataStream.BaseStream.Length - position;
            }
        ...
        private IEnumerable<Atom> ReadAtoms(BinaryReader dataStream)
        {
            while (true)
            {
                ...
                else if (dataStream.BaseStream.Position > dataStream.BaseStream.Length)
                {
                    yield break;
                }
        ...
        public void Process(string infilename, Stream outStream, long limit = long.MaxValue, bool to_end = false, bool cleanup = true)
        {
            using var fileStream = new FileStream(infilename, FileMode.Open, FileAccess.Read, FileShare.Read);
            using var datastream = new BinaryReader(fileStream);
            // Get the top level atom index
            var index = GetIndex(datastream).ToList();
            ...
            foreach (Atom atom in index)
            {
                if (atom.Name == "ftyp")
                {
                    ...
                    outStream.Write(buffer, 0, (int)atom.Size);
                }
            }

            if (!to_end)
            {
                WriteMoov(moov, outStream);
            }
        ...
        private void WriteMoov(MemoryStream memoryStream, Stream outStr)
        {
